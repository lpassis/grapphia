...
public class gameController2 : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		...
		if (xboard_letter >= -0.8f)
			xboard_letter -= 0.100f; // velocidade da caixa 1
		else if (yboard_letter <= -3.0f) {
			yboard_letter += 0.090f;

		}

		//if (xboard2_letter >= 0f) xboard2_letter -= 0.065f;
		if (xboard2_letter >= 1.7f)
			xboard2_letter -= 0.100f; // velocidade da caixa 2
		else if (yboard2_letter <= -3.0f) {
